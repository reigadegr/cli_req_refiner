...
async fn try_upstreams(plan: ProxyPlan, ctx: RetryContext<'_>) -> RetryLoopResult {
    ...
    for attempt in 1..=max_attempts {
        ...
        let api_key = if selected_upstream.api_keys.is_empty() {
            ""
        } else {
            let key_index =
                (selected_upstream.api_key_index + attempt - 1) % selected_upstream.api_keys.len();
            &selected_upstream.api_keys[key_index]
        };
...
fn select_upstream(
    ...
    let (index, name, base_url, models, api_keys, api_key_index, user_agent, mode) =
